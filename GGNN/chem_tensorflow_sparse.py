...
class SparseGGNNChemModel(ChemModel):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        for layer_idx in range(len(self.params['layer_timesteps'])):
            with tf.variable_scope('gnn_layer_%i' % layer_idx):
                ...
                edge_weights = tf.reshape(edge_weights, [self.num_edge_types, h_dim, h_dim])  # shape[N, D, D]
                ...
                for step in range(num_timesteps):
                    with tf.variable_scope('timestep_%i' % step):
                        ...
                        for edge_type_idx, adjacency_list_for_edge_type in enumerate(
                            ...
                            messages.append(all_messages_for_edge_type)  # shape[N, E, D]
                        ...
                        if self.params['use_edge_msg_avg_aggregation']:
                            ...
                            incoming_messages /= num_incoming_edges + SMALL_NUMBER  # 归一化
    ...
    def gated_regression(self, last_h, regression_gate, regression_transform):
        ...
        return tf.nn.sigmoid(tf.squeeze(tf.sparse_tensor_dense_matmul(graph_nodes, gated_outputs), axis=[-1]))  # [g]
    ...
    def __graph_to_adjacency_lists(self, graph) -> Tuple[Dict[int, np.ndarray], Dict[int, Dict[int, int]]]:
        ...
        return final_adj_lists, num_incoming_edges_dicts_per_type  # shape [N, 2], shape [N, V, 1]
